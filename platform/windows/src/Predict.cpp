...
std::wstring CTextService::MaybeRestoreDiacritics(const std::wstring& text) {
  if (!autoDiacritics_ || text.empty()) return text;
  ...
  if (end == 0 || end > kMaxWordChars) return text;
  ...
  for (wchar_t c : word) {
    if (!IsAsciiLetter(c)) return text;
  }
  ...
  if (reply.empty()) return text;  // host không đủ bằng chứng
  ...
  if (restored.size() != word.size()) return text;
